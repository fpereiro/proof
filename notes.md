...
### Reconstruction of the universal Turing machine & review from the beginning
...
**Review of terms:**
   ...
   - *universal Turing machine (utm)*: the machine that can interpret any other machine.

Because the utm requires the executed/interpreted machine to be encoded, and that encoding relies on the machine being written in strict form, we need to find an algorithm to convert the three machines presented in the paper (zeroes & ones, increasing sequences of 1s, and the utm itself with all its helper functions) into strict form so that then they can be encoded and executed/interpreted by the utm.
...
Instead of expanding now the utm, let's understand it first by studying how it can interpret the two machines we saw above. For this, we need to encode these machines so that they can be placed in the tape that the utm will read.

#### Encoding machines so that they can be executed by the utm
...
Let's encode the first machine:
...
There's no double colon symbol in unicode, so let's use the Ethiopic Colon instead: `፥`.
